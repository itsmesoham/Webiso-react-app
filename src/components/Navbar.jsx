                    ...
                    <div className="navbar-menu desktop-menu">
                        {menuItems.map((item, index) => (
                            <div
                                key={index}
                                className="menu-item"
                                onMouseEnter={() => setOpenDropdowns(prev => ({ ...prev, desktop: item.title }))}
                                onMouseLeave={() => setOpenDropdowns(prev => ({ ...prev, desktop: null }))}
                            >
                                <a href={item.path} className="menu-link">
                                    {item.title}
                                    {item.submenu && (
                                        <RiArrowDownSLine className={`menu-icon ${openDropdowns.desktop === item.title ? 'rotated' : ''}`} />
                                    )}
                                </a>
                            ...
                            {menuItems.map((item, index) => (
                                <div key={index} className="mobile-menu-item">
                                    <div
                                        ...
                                        onClick={() => item.submenu && toggleMobileDropdown(item.title)}
                                    >
                                        <span className="mobile-menu-title">
                                            {item.title}
                                            {item.submenu && (
                                                <RiArrowDownSLine
                                                    className={`menu-icon ${openDropdowns.mobile.has(item.title) ? 'rotated' : ''}`}
                                                />
                                            )}
                                        </span>
                                    </div>
